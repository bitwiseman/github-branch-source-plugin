...
public class GitHubSCMProbeTest {
  ...
  @Rule
  public WireMockRule githubApi =
      factory.getRule(
          WireMockConfiguration.options()
              .dynamicPort()
              .usingFilesUnderClasspath("cache_failure")
              .extensions(
                  ResponseTemplateTransformer.builder().global(true).maxCacheEntries(0L).build()));
  ...
  public void statWhenRootIs404() throws Exception {
    githubApi.stubFor(
        get(urlEqualTo("/repos/cloudbeers/yolo/contents/?ref=refs%2Fpull%2F1%2Fmerge"))
  ...
  public void statWhenDirIs404() throws Exception {
    githubApi.stubFor(
        get(urlEqualTo("/repos/cloudbeers/yolo/contents/subdir?ref=refs%2Fpull%2F1%2Fmerge"))
            .willReturn(aResponse().withStatus(404))
            .atPriority(0));
