...
public class GithubAppCredentialsTest extends AbstractGitHubWireMockTest {
	...
	public void setUpWireMock() throws Exception {
		...
		githubApi.stubFor(get(urlEqualTo("/app")).willReturn(aResponse().withHeader("Content-Type",
		                                                                            "application/json; charset=utf-8")
		                                                                .withBodyFile("../AppCredentials/files/body-mapping-githubapp-app.json")));
		githubApi.stubFor(get(urlEqualTo("/app/installations")).willReturn(aResponse().withHeader("Content-Type",
		                                                                                          "application/json; charset=utf-8")
		...
		githubApi.stubFor(post(urlEqualTo("/app/installations/654321/access_tokens")).inScenario(scenarioName)
		                                                                             ...
		                                                                             .willSetStateTo("1")
		                                                                             .withRequestBody(equalToJson("{\"permissions\":{\"pull_requests\":\"write\",\"metadata\":\"read\",\"checks\":\"write\",\"contents\":\"read\"}}",
		                                                                                                          true,
		                                                                                                          false))
	...
	public void testAgentRefresh() throws Exception {
		...
		try {
			...
			job.addProperty(new ParametersDefinitionProperty(new StringParameterDefinition("REPO",
			                                                                               sampleRepo.toString())));
			...
			assertThat("Creds should cache on master, pass to agent, and refresh agent from master once",
			           ...
			           contains(
			                    ...
			                    // (agent log added out of order) "Keeping cached GitHub App Installation Token for app
			                    // ID
