...
public class GithubSCMSourceBranchesTest extends GitSCMSourceBase {
	...
	public void testMissingSingleBranch() throws IOException {
		...
		githubApi.stubFor(get(urlEqualTo("/repos/cloudbeers/yolo/branches/non-existent-branch")).willReturn(aResponse().withStatus(404)
	...
	public void testExistentSingleBranch() throws IOException {
		...
		githubApi.stubFor(get(urlEqualTo("/repos/cloudbeers/yolo/branches/existent-branch")).willReturn(aResponse().withHeader("Content-Type",
		                                                                                                                       "application/json; charset=utf-8")
	...
	public void testExistingMultipleBranchesWithDefaultInPosition1() throws IOException {
		...
		githubApi.stubFor(get(urlEqualTo("/repos/cloudbeers/yolo/branches")).willReturn(aResponse().withHeader("Content-Type",
		                                                                                                       "application/json; charset=utf-8")
		...
		GitHubSCMSourceRequest request = context.newRequest(new GitHubSCMSource("cloudbeers", "yolo", null, false),
		                                                    null);
		Iterator<GHBranch> branches = new GitHubSCMSource.LazyBranches(request, repo).iterator();
		// Expected: In the iterator will be a multiple branches named nexistent-branch1(because it is alphabetically
		// sorted first)
		// and master
		assertTrue(branches.hasNext());
