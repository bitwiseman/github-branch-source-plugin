...
public class GithubSCMSourcePRsTest extends GitSCMSourceBase {
	...
	public void testClosedSinglePR() throws IOException {
		...
		githubApi.stubFor(get(urlEqualTo("/repos/cloudbeers/yolo/pulls/1")).willReturn(aResponse().withHeader("Content-Type",
		...
		GitHubSCMSourceRequest request = context.newRequest(new GitHubSCMSource("cloudbeers", "yolo", null, false),
		                                                    null);
		Iterator<GHPullRequest> pullRequest = new GitHubSCMSource("cloudbeers",
		                                                          "yolo",
		                                                          null,
	...
	public void testSinglePRThrowingExceptionOnGettingNumbers() throws Exception {
		...
		try {
			Iterator<GHPullRequest> pullRequest = new GitHubSCMSource("cloudbeers",
			                                                          "yolo",
			                                                          null,
			                                                          false).new LazyPullRequests(request,
			                                                                                      mockRequest).iterator();
			fail();
		} catch (Exception e) {
	...
	public void testOpenSinglePRThrowsFileNotFoundOnObserve() throws Exception {
		...
		GitHubSCMSourceRequest request = context.newRequest(new GitHubSCMSource("cloudbeers", "yolo", null, false),
		                                                    null);
		Iterator<GHPullRequest> pullRequestIterator = new GitHubSCMSource("cloudbeers",
		                                                                  "yolo",
		                                                                  null,
		                                                                  false).new LazyPullRequests(request,
		                                                                                              repoSpy).iterator();
	...
	public void testOpenMultiplePRs() throws IOException {
		...
		githubApi.stubFor(get(urlEqualTo("/repos/cloudbeers/yolo/pulls?state=open")).willReturn(aResponse().withHeader("Content-Type",
		                                                                                                               "application/json; charset=utf-8")
	...
	public void testOpenMultiplePRsWithMasterAsOrigin() throws IOException {
		...
		githubApi.stubFor(get(urlEqualTo("/repos/cloudbeers/yolo/pulls?state=open&head=cloudbeers%3Amaster")).willReturn(aResponse().withHeader("Content-Type",
		                                                                                                                                        "application/json; charset=utf-8")
		                                                                                                                            .withBodyFile("../PRs/_files/body-yolo-pulls-open-multiple-PRs.json")));
		SCMHeadObserver mockSCMHeadObserver = Mockito.mock(SCMHeadObserver.class);
		GitHubSCMSourceContext context = new GitHubSCMSourceContext(null, mockSCMHeadObserver);
