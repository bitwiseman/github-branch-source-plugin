...
public class GithubSCMSourceTagsTest extends GitSCMSourceBase {
	...
	public void testMissingSingleTag() throws IOException {
		...
		Mockito.when(mockSCMHeadObserver.getIncludes())
		       .thenReturn(Collections.singleton(new GitHubTagSCMHead("non-existent-tag", System.currentTimeMillis())));
	...
	public void testExistingMultipleTags() throws IOException {
		...
		Mockito.when(mockSCMHeadObserver.getIncludes())
		       .thenReturn(new HashSet<>(Arrays.asList(new GitHubTagSCMHead("existent-multiple-tags1",
		                                                                    System.currentTimeMillis()),
		                                               new GitHubTagSCMHead("existent-multiple-tags2",
		                                                                    System.currentTimeMillis()))));
		GitHubSCMSourceContext context = new GitHubSCMSourceContext(null, mockSCMHeadObserver);
		context.wantTags(true);
		GitHubSCMSourceRequest request = context.newRequest(new GitHubSCMSource("cloudbeers", "yolo", null, false),
		...
		// Expected: When getting the tags, we should get both tags and then we are failing on trying to get a 3rd tag
		// or remove the iterator
		assertTrue(tags.hasNext());
	...
	public void testExistingMultipleTagsGHFileNotFoundExceptionIterable() throws IOException {
		...
		// Expected: When initially getting multiple tags, there will then be a thrown filenotfound which returns an
		// empty list
	...
	public void testExistingMultipleTagsIteratorGHFileNotFoundExceptionOnHasNext() throws IOException {
		...
		// Expected: When initially getting multiple tags, return a filenotfound on hasNext which means it will get an
		// empty list
	...
	public void testExistingMultipleTagsIteratorGHExceptionOnHasNextButThrowsFileNotFoundOnGetRefs()
		...
		// Expected: First call to hasNext throws a GHException and then returns a FileNotFound on getRefs so it returns
		// an empty list
		assertFalse(tags.hasNext());
	}
	...
	public void testExistingMultipleTagsIteratorGHExceptionOnHasNextButThrowsIOErrorOnGetRefs() throws IOException {
		...
		// Expected: First call to hasNext throws a GHException and then returns a IO exception on getRefs so it returns
		// an error
		try {
			tags.hasNext();
			fail("This should throw an exception");
